...
router.get("/", async (_req, res, next) => {
  try {
    let results=await ConcertsRepository.getAll({hidden:false, dates:QueryOptions.DATES.FUTURE, limit:6});
...
router.get("/news", async (req, res, next) => {
  try {
    ...
    let news = await NewsRepository.getAll({hidden:false, offset, limit:itemCount});
...
router.get("/api/concerts", async (req, res, next) => {
  try {
    let concerts = await ConcertsRepository.getAll({hidden:false, dates:QueryOptions.DATES.ALL});
    res.statusCode = 200;
    res.json(concerts);
  } catch (error) {
    next(error);
