...
const defaultOptions = {hidden:false,dates:queryOptions.DATES.ALL, order:queryOptions.ORDER.DESC, limit:0, offset:0, search:''};

//concerts repository
let ConcertsRepository = {
    async getAll(options) {
        options=Object.assign({},defaultOptions,options);
        let sqlSelectDateCondition = queryOptions.sqlSelectDateCondition(options.dates);
        ...
        if(sqlSelectDateCondition!='') whereClause+=` ${sqlSelectDateCondition}`;
    ...
    async getById(id) {
        ...
        if (results.length>0){
            results[0].description = viewhelpers.UnescapeQuotes(results[0].description);
            return results[0];
        }
        return null;
    },
